...
def from_location_bubble(bubble_name):
    ...
    fb = open(f'I:/d2_output_3_0_2_0/{gf.get_pkg_name(bubble_file)}/{bubble_file}.bin', 'rb').read()
    lower_file = gf.get_file_from_hash(fb[0x8:0xC].hex())
    fb = open(f'I:/d2_output_3_0_2_0/{gf.get_pkg_name(lower_file)}/{lower_file}.bin', 'rb').read()
    ...
    for x in all_top_maps:
        fb = open(f'I:/d2_output_3_0_2_0/{gf.get_pkg_name(x)}/{x}.bin', 'rb').read()
        f = gf.get_file_from_hash(fb[0x54:0x58].hex())
        fb = open(f'I:/d2_output_3_0_2_0/{gf.get_pkg_name(f)}/{f}.bin', 'rb').read()
        if len(fb) < 0xD8:
            continue
        ...
        fb = open(f'I:/d2_output_3_0_2_0/{gf.get_pkg_name(map_file)}/{map_file}.bin', 'rb').read()
        flt = [struct.unpack('f', fb[0x80+4*i:0x80+4*(i+1)])[0] for i in range(3)]

        print(map_file, len(fb), f'xyz {flt}')
...
if __name__ == '__main__':
    ...
    from_location_bubble('bubble|city_tower_d2|armory')