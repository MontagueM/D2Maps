...
def get_hex_from_pkg(d2map: Map):
    main_hex = gf.get_hex_data(f'I:/d2_output_3_0_0_1/{d2map.get_pkg_name()}/{d2map.name}.bin')
    file_hash = main_hex[24*2:24*2+8]
    scales_file = met.File(name=gf.get_file_from_hash(file_hash))
    d2map.scales_hex = gf.get_hex_data(f'I:/d2_output_3_0_0_1/{scales_file.get_pkg_name()}/{scales_file.name}.bin')[48 * 2:]
...
if __name__ == '__main__':
    ...
    unpack_folder('city_tower_d2_02aa', shaders=False)
